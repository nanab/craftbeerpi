...
def loadRecipe(id):
    ...
    for a in recipe.steps:
        s = Step(name=a.name, order=a.order, timer=a.timer, temp=a.temp, type=a.type, state="I", kettleid=a.kettleid, wateramountfill=a.wateramountfill, switchid=a.switchid, switchstate=a.switchstate)
        db.session.add(s)
        db.session.commit()
...
def save_book():
    ...
    steps = []
    for a in s:
        steps.append(RecipeBookSteps(name=a.name, order=a.order, timer=a.timer, temp=a.temp, type=a.type, kettleid=a.kettleid, wateramountfill=a.wateramountfill, switchid=a.switchid, switchstate=a.switchstate))
